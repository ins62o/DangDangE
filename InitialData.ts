import disoneBlood from "./assets/image/disoneBlood.png";
import oneBlood from "./assets/image/oneBlood.png";
import distwoBlood from "./assets/image/distwoBlood.png";
import twoBlood from "./assets/image/twoBlood.png";
import disthreeBlood from "./assets/image/disthreeBlood.png";
import threeBlood from "./assets/image/threeBlood.png";
import disfourBlood from "./assets/image/disfourBlood.png";
import fourBlood from "./assets/image/fourBlood.png";

export const BloodData = [
  {
    id: "1",
    title: "1형 당뇨",
    info: "당뇨병의 2% 미만을 차지하며 30세 전에 진단되는 경우가 많지만 성인에서도 나타날 수 있습니다.",
    offimage: disoneBlood,
    onimage: oneBlood,
  },
  {
    id: "2",
    title: "2형 당뇨",
    info: "당뇨병의 대부분이 2형 당뇨이며, 보통 40세 이상에서 발생하지만 최근에는 젊은 환자가 늘고 있습니다.",
    offimage: distwoBlood,
    onimage: twoBlood,
  },
  {
    id: "3",
    title: "기타 당뇨",
    info: "유전자 결함, 유전질환, 약물, 감염에 의해 발생하는 당뇨병입니다.",
    offimage: disthreeBlood,
    onimage: threeBlood,
  },
  {
    id: "4",
    title: "임신 당뇨",
    info: "임신 중에 발견된 당뇨병이며 임신 기간은 물론, 출산 후에도 장기적으로 예방 조치를 취해야 합니다.",
    offimage: disfourBlood,
    onimage: fourBlood,
  },
];

export const motivationalMessages = [
  "작은 변화가 큰 건강을 만듭니다.",
  "당당하게, 건강한 혈당을 지켜요.",
  "오늘의 한 걸음이 내일의 건강을 만듭니다.",
  "당신의 건강, 당신의 손안에 있습니다.",
  "꾸준함이 건강을 바꿉니다.",
  "한 걸음씩, 당당하게 목표를 향해!",
  "혈당 관리, 당당하게 해결해요.",
  "매일의 작은 변화로 큰 성과를 만들어가요.",
  "건강한 혈당, 당신의 선택입니다.",
  "작은 습관 변화가 큰 건강을 만듭니다.",
  "당당하게 나아가며 더 건강한 나를 만나요!",
  "혈당 관리는 이제 당당하게! 함께 시작해요.",
  "오늘도 건강한 선택을, 당당하게!",
  "한 걸음, 한 걸음이 건강을 만들어갑니다.",
  "당당하게 관리하고, 더 건강한 내일을!",
  "꾸준함이 건강을 지킵니다. 함께 해요!",
  "혈당 관리의 시작은 당당함에서!",
  "건강한 혈당, 당당하게 챙기세요.",
  "작은 변화가 당신의 건강을 지킵니다.",
  "오늘의 나를 당당하게 만들어요!",
];

export const times = [
  "🌚 식전 혈당(공복)",
  "🔆 아침 식후 2시간 혈당",
  "⛅ 점심 식후 2시간 혈당",
  "🌙 저녁 식후 2시간 혈당",
  "🛏 취침 전 혈당",
];
